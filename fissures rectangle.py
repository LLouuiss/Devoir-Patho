def calcul_caracteristiques_section_non_fissuree(b1, h1, d, d_prime, A_s, A_s_prime, E_cm, phi_eff, E_s):
    """
    ...
    y_G_I = (b1*h1*h1/2 + alpha_e * A_s * d + alpha_e * A_s_prime * d_prime) / (b1 * h1 + alpha_e * A_s + alpha_e * A_s_prime)
    
    # Calcul de l'inertie de la section non fissurée
    I_I = (b1 * h1**3 / 12)  + b1 * h1 * (h1/2 - y_G_I)**2 + alpha_e * A_s * (y_G_I - d)**2 + alpha_e * A_s_prime * (y_G_I - d_prime)**2
    
    return y_G_I, I_I,alpha_e

def calcul_caracteristiques_section_fissuree(b1, h1,x, d, d_prime, A_s, A_s_prime, alpha_e):
    """
    ...
    I_II = (b1 * x**3 / 12) +b1*x*(x-h1/2)**2   + alpha_e * A_s * (x - d)**2 + alpha_e * A_s_prime * (x - d_prime)**2
...
phi = 0.016 #m diamètre des armatures
y_G_I, I_I,alpha_e = calcul_caracteristiques_section_non_fissuree(b1, h1, d, d_prime, A_s, A_s_prime, E_cm, phi_eff, E_s)
y_G_II, I_II = calcul_caracteristiques_section_fissuree(b1, h1,0.2, d, d_prime, A_s, A_s_prime, alpha_e)
print("y_G_I = ",y_G_I)
print("I_I = ",I_I)
print("alpha_e = ",alpha_e)
print("y_G_II = ",y_G_II)
print("I_II = ",I_II)
w = calcul_ouverture_fissures(M, d, y_G_II, I_II, A_s, b1, h1, E_s, alpha_e,k_t,f_ctm,c,k1,k2,phi)
print("w [mm], si <0.3 -> OK = ",w)
print("Pour avoir les memes valeurs que dans le cours, il faut modifier la valeur de I_II par celle du tp car la valeur de I_II est fausse dans le corrige du tp")